...
export async function handleUpdateMessage() {
    ...
    if (last_version === "" || last_version === game.modules.get(MODULE_ID).version) return;
    const updateMessage = {
    ...
    game.settings.set(MODULE_ID, 'last-version', game.modules.get(MODULE_ID).version)
    ChatMessage.create({
        content: renderTemplate(`modules/${MODULE_ID}/templates/updateMessage.hbs`, updateMessage),
